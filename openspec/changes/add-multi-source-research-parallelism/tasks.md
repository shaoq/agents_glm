...
- [ ] 4.1 `build_production_coordinator` 暴露 sibling adapter 注入点（`recall_fn`/`query_fn`/`fetch_fn`）；`build_production_coordinator_from_settings` 暂不接真实 sibling（保留注入缝，待后续 change）
- [ ] 4.2 装配多源 research handler（`EVIDENCE_RESEARCHER` → 新 handler），移除 `_LLMResearchHandler`
- [ ] 4.3 收紧 `allowed_capabilities`：从 `frozenset(CapabilityKind)` 改成实际支持集（`{RAG_SEARCH, MEMORY_RECALL, WEB_RESEARCH?}`）
- [ ] 4.4 `WorkerDefinition.allowed_capabilities` 配齐（满足 `Router` 的 allowlist 校验）
- [ ] 4.5 集成测试：经 `build_production_coordinator` 注入 fake double，跑通 RESEARCH 多源链路
...
- [ ] 6.1 集成：RESEARCH 多源 → EvidenceJoin → ANALYZE 全链路（经 `build_production_coordinator` 注入 fake double）
- [ ] 6.2 集成：多源 research task 并发执行验证
- [ ] 6.3 集成：`web_enabled=False` 时 web 源被映射过滤、task 不带该源
- [ ] 6.4 架构测试：生产 composition 不含任何 Fake 类、不 import 真实 sibling / 网络栈；测试 double 全部位于 `tests/`
- [ ] 6.5 覆盖率：新增/改动模块达 80%+

## 7. 文档

- [ ] 7.1 更新 `agents_orchestration/README.md`：RESEARCH 多源骨架 + 测试 double 说明 + 真实 Memory/RAG/Web 接线 deferred 备注
