...
agents_rag 是一个面向**领域/企业文档问答**的 RAG（检索增强生成）工程。基于智谱 GLM 全家桶（LLM + Embedding + Rerank 全部走云端 API），支持 PDF / Markdown / Office / 网页 多格式文档，采用**混合检索（向量 + BM25）+ Rerank 重排**，强调准确性、引用溯源与抗幻觉，按 **CLI → FastAPI → Web UI** 渐进式交付，架构支持「先小后大」扩展。
...
- 结构感知分块 + 父子分块
- 混合检索（向量 + BM25）+ RRF 融合 + 智谱 Rerank 精排
...
| LLM | 智谱 GLM-4.5（主力，可升级 GLM-5.2）/ Flash 系列（低成本高频） |
| Embedding | 智谱 embedding-3，默认 2048 维（可自定义 256–2048） |
| Rerank | 智谱 Rerank API（文本重排序） |
| 框架路线 | 自研混合（自研管线编排 + 专门库，不绑大框架） |
| 解析器 | Docling 主力 + MinerU 兜底复杂 PDF + OCR 扫描件 |
...
| 层 | 选型 | 备选 | 说明 |
|----|------|------|------|
| LLM | GLM-4.5（可升 GLM-5.2） | GLM-4.5-Flash | 主力用 4.5，高频低成本用 Flash |
| Embedding | embedding-3（2048 维） | embedding-2 | 256–2048 可选，8K 上下文，0.5 元/百万 tokens |
| Rerank | 智谱 Rerank API | 本地 bge-reranker-v2-m3 | 原生零运维，无需 GPU |
...
| 文档解析 | Docling + MinerU 兜底 | Unstructured | 见 §13 难点 1 |
...
| SDK | zhipuai | zai-sdk（官方新版） | OpenAI 兼容接口 |
...
### Rerank
- 智谱「文本重排序」API：输入 query + candidate 文本列表，返回各候选相关性分数
...
| `ParserRouter` | `route(file) -> Parser` | 按扩展名 + 内容探测路由，失败降级 |
| `Chunker` | `chunk(doc) -> list[Chunk]` | 结构感知 + 父子分块 |
| `Embedder` | `embed(texts) -> list[Vec]` | 智谱 embedding-3，批量 + 缓存 |
...
| `Reranker` | `rerank(query, candidates) -> ranked` | 智谱 Rerank API |
...
- **解析失败**：`ParserRouter` 降级到备选 parser；仍失败则跳过该文件并记录结构化日志，不中断批量
- **OCR 兜底**：PDF 解析出文本过少时判定为扫描件，走 OCR
- **检索为空**：返回「未找到相关内容」，不硬编答案
- **引用校验失败**：生成的引用编号若未命中检索结果，剔除并标记
- **密钥缺失**：启动时校验 `ZHIPUAI_API_KEY`，缺失即 fail-fast 报错
...
1. **文档解析（质量上限）**：Docling 主力（版面+表格结构化输出强），复杂中文/学术 PDF 用 MinerU 兜底，扫描件走 RapidOCR；`ParserRouter` 屏蔽格式差异并提供降级。
2. **分块策略**：结构感知分块（按标题/段落/表格单元，不切断语义）+ 父子分块（检索小块、回传带父块上下文）；chunk_size/overlap 可配置。
3. **检索质量**：向量 + BM25 双路召回（专有名词/型号/编号靠 BM25）→ RRF 融合 → 智谱 Rerank 精排。
...
2. `parsing/`（base → docling → office → html → router；mineru/ocr 后置）
3. `chunking/`（structural → parent_child）
4. `indexing/embedder`（智谱 embedding-3，批量+缓存）
