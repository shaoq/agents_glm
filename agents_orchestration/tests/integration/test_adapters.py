...
# --- 7.1 real adapters via the router --------------------------------------


@pytest.mark.integration
async def test_registry_round_trip_memory_rag(backend) -> None:
    registry = build_test_registry()
    router = CapabilityRouter(registry, backend.idgen)
    researcher = WorkerRegistry.default().get(WorkerRole.EVIDENCE_RESEARCHER)

    rag = await router.invoke(_req("rag"), worker=researcher, run_policy=_policy())
    memory = await router.invoke(_req("memory"), worker=researcher, run_policy=_policy())
...
@pytest.mark.integration
async def test_web_denied_when_policy_disabled(backend) -> None:
    registry = build_test_registry()
    router = CapabilityRouter(registry, backend.idgen)
    researcher = WorkerRegistry.default().get(WorkerRole.EVIDENCE_RESEARCHER)
    result = await router.invoke(
        _req("web", url="https://example.com/x"),
        worker=researcher,
        run_policy=_policy(),  # web_enabled defaults to False
    )
    assert not result.succeeded


@pytest.mark.integration
async def test_web_allowed_when_policy_enabled_and_domain_permitted(backend) -> None:
    registry = build_test_registry()
    router = CapabilityRouter(registry, backend.idgen)
    researcher = WorkerRegistry.default().get(WorkerRole.EVIDENCE_RESEARCHER)
    result = await router.invoke(
...
def test_capability_doctor_reports_all_without_secrets() -> None:
    ...
    assert kinds == {
        CapabilityKind.MEMORY_RECALL.value,
        CapabilityKind.RAG_SEARCH.value,
        CapabilityKind.WEB_RESEARCH.value,
    }
    for entry in report:
